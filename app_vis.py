...
REQUIRED_COLUMNS = ["DATE/TIME", "PIMn", "SLEEP_STATE"]
HISTORY_LIMIT = 30
...
# -----------------------------
# Working data and derived views
# -----------------------------
...
plot_signal = st.sidebar.selectbox(
    ...
    index=[col for col in st.session_state.df.columns].index("PIMn") if "PIMn" in st.session_state.df.columns else 0,
)
...
def step_period(step: int):
    current_idx = all_periods.index(st.session_state.selected_period)
    new_idx = min(max(current_idx + step, 0), len(all_periods) - 1)
    st.session_state.selected_period = all_periods[new_idx]


nav_col1, nav_col2, nav_col3 = st.columns([1, 3, 1])
with nav_col1:
    st.button("⬅ Previous", on_click=step_period, args=(-1,), use_container_width=True)
with nav_col2:
...
with nav_col3:
    st.button("Next ➡", on_click=step_period, args=(1,), use_container_width=True)
...
period_seg_df = seg_df[seg_df["display_period"] == selected_period].copy() if not seg_df.empty else pd.DataFrame()
...
with st.sidebar:
    ...
    if hist_col1.button("Undo", use_container_width=True, disabled=not st.session_state.undo_stack):
        if undo_last():
            st.rerun()
    if hist_col2.button("Redo", use_container_width=True, disabled=not st.session_state.redo_stack):
        if redo_last():
            st.rerun()
    ...
    st.header("Auto cleanup")
    with st.form("cleanup_form"):
        bridge_gap_min = st.number_input("Bridge wake gaps up to (minutes)", min_value=0, max_value=240, value=5, step=1)
        remove_seg_min = st.number_input("Remove sleep segments shorter than (minutes)", min_value=0, max_value=240, value=10, step=1)
    ...
    if cleanup_submit:
        ...
        st.success(f"Cleanup applied. Bridged {gap_changes} rows and removed {seg_changes} rows.")
        st.rerun()

    st.header("Segment actions")
    segment_options = [] if seg_df.empty else seg_df["segment_id"].astype(int).tolist()

    with st.expander("Adjust boundaries", expanded=True):
        if segment_options:
            selected_seg = st.selectbox("Segment", segment_options, key="adjust_segment")
            ...
            with st.form("adjust_form"):
                ...
                with col1:
                    start_date = st.date_input("Start date", value=default_start.date(), key="adj_start_date")
                    start_time = st.time_input("Start time", value=default_start.time(), key="adj_start_time", step=60)
                with col2:
                    end_date = st.date_input("End date", value=default_end.date(), key="adj_end_date")
                    end_time = st.time_input("End time", value=default_end.time(), key="adj_end_time", step=60)
            ...
            if adjust_submit:
                ...
                if not ok:
                    ...
                    warning = overlap_warning(seg_df, selected_seg, new_start, new_end)
                    if warning:
                        st.warning("The updated range overlaps another existing segment. They will merge after recalculation.")
                    st.rerun()
        else:
    ...
    with st.expander("Create new segment"):
        ...
        with st.form("create_form"):
            ...
            with cc1:
                c_start_date = st.date_input("New start date", value=default_create_start.date(), key="new_seg_start_date")
                c_start_time = st.time_input("New start time", value=default_create_start.time(), key="new_seg_start_time", step=60)
            with cc2:
                c_end_date = st.date_input("New end date", value=default_create_end.date(), key="new_seg_end_date")
                c_end_time = st.time_input("New end time", value=default_create_end.time(), key="new_seg_end_time", step=60)
    ...
    with st.expander("Merge segments"):
        if segment_options:
            merge_ids = st.multiselect("Segments to merge", options=segment_options)
            if len(merge_ids) >= 2:
                ...
                st.caption(f"This will mark the full range from {min_start} to {max_end} as sleep.")
                st.caption(f"Total covered range: {bridge_min} minutes")
            if st.button("Merge selected", use_container_width=True, disabled=len(merge_ids) < 2):
    ...
    with st.expander("Remove segment"):
        if segment_options:
            rm_id = st.selectbox("Segment to remove", options=segment_options, key="remove_segment")
            rm_row = seg_df[seg_df["segment_id"] == rm_id].iloc[0]
            st.caption(f"Duration: {rm_row['duration_min']} min")
            if st.button("Remove selected segment", use_container_width=True):
